...
export default function App() {
  ...
  // Firebase 연결 및 실시간 구독
  useEffect(() => {
    const gameStateRef = ref(database, `rooms/${DEFAULT_ROOM_ID}/gameState`);
    ...
    const connUnsubscribe = onValue(connectedRef, (snapshot) => {
  ...
  const leaveTeam = async () => {
    const updatedTeams = gameState.teams.map(t => ({
      ...t,
      members: t.members.filter(m => m.clientId !== myClientId)
    }));
  ...
  const joinGame = async () => {
    ...
    const existingTeamIndex = gameState.teams.findIndex(t => t.groupNumber === joinGroup);
    let newState: GameState;

    if (existingTeamIndex !== -1) {
      const existingTeam = gameState.teams[existingTeamIndex];
      if (existingTeam.members.some(m => m.clientId === myClientId)) {
        setViewMode('USER');
        return;
      }
      ...
      const updatedTeam = {
        ...existingTeam,
        members: [...existingTeam.members, { clientId: myClientId, name: joinName }]
      };

      const updatedTeams = [...gameState.teams];
      updatedTeams[existingTeamIndex] = updatedTeam;
      updatedTeams.sort((a, b) => a.groupNumber - b.groupNumber);

      newState = {
        ...gameState,
        teams: updatedTeams,
        logs: [...gameState.logs, `[SYSTEM] '${joinName}' 님이 ${joinGroup}조에 합류했습니다.`]
      };
    } else {
      if (gameState.teams.length >= gameState.roomConfig.maxTeams) {
        alert("최대 팀 수에 도달했습니다.");
        return;
      }
      ...
      const newTeam: Team = {
        id: generateId(),
        name: `${joinName} (대표)`,
        groupNumber: joinGroup,
        chips: STARTING_CHIPS,
        cards: [],
        score: STARTING_CHIPS,
        lastAction: null,
        members: [{ clientId: myClientId, name: joinName }]
      };

      newState = {
        ...gameState,
        teams: [...gameState.teams, newTeam].sort((a, b) => a.groupNumber - b.groupNumber),
        logs: [...gameState.logs, `[SYSTEM] ${joinGroup}조 ('${newTeam.name}') 등록 완료.`]
      };
    }
  ...
  // 게임 액션 (PASS/TAKE)
  const sendAction = async (type: 'ACTION_PASS' | 'ACTION_TAKE') => {
    if (!myTeam) return;
    if (gameState.phase !== GamePhase.PLAYING) return;

    const currentTeam = gameState.teams[gameState.currentTurnIndex];
    if (currentTeam.id !== myTeam.id) return;
    ...
    if (type === 'ACTION_PASS') {
      ...
      newState = {
        ...
        pot: gameState.pot + 1,
        teams: updatedTeams,
        currentTurnIndex: (gameState.currentTurnIndex + 1) % gameState.teams.length,
        logs: [...gameState.logs, `[BID] ${currentTeam.groupNumber}조: 입찰 포기 (자원 1억 지불)`]
      };
    } else {
      const takenCard = gameState.currentCard!;
      const takenPot = gameState.pot;
      ...
      const logs = [...gameState.logs, `[SUCCESS] ${currentTeam.groupNumber}조: ${takenCard}억 프로젝트 수주 (자원 +${takenPot}억)`];
  ...
  const ConnectionStatus = () => (
    <div className={`fixed bottom-4 left-4 z-50 flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-mono ${isConnected ? 'bg-green-900/80 text-green-400' : 'bg-red-900/80 text-red-400'}`}>
